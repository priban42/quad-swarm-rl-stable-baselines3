                ...
                if isinstance(info, list):
                    if any(done):
                        for i in range(len(info)):
                            info[i]["terminal_observation"] = observation[i]
...
class SubprocVecEnvCustom(VecEnv):
    ...
    def __init__(self, env_fns: list[Callable[[], gym.Env]], start_method: Optional[str] = None, agents_per_env=1):
        ...
        super().__init__(len(env_fns)*agents_per_env, observation_space, action_space)
    ...
    def step_async(self, actions: np.ndarray) -> None:
        ...
        for i in range(len(self.remotes)):
            self.remotes[i].send(("step", actions[i*self.agents_per_env:(i+1)*self.agents_per_env, :]))
        self.waiting = True
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        return _stack_obs(obs, self.observation_space), _stack_list_2d(rews).flatten(), _stack_list_2d(dones).flatten(), [info for info_tuple in infos for info in info_tuple]  # type: ignore[return-value]
    ...
    def _get_indices(self, indices: VecEnvIndices) -> Iterable[int]:
        ...
        """
        if indices is None:
            indices = range(self.num_envs//self.agents_per_env)
...
def _stack_list_2d(input_arr):
    stacked_list = np.stack(input_arr)
    stacked_list = stacked_list.reshape(-1, stacked_list.shape[-1]) if stacked_list.ndim == 3 else stacked_list
    return stacked_list
...
def _stack_obs(obs_list: Union[list[VecEnvObs], tuple[VecEnvObs]], space: spaces.Space) -> VecEnvObs:
    ...
    if isinstance(space, spaces.Dict):
        ...
        return _stack_list_2d(obs_list)
