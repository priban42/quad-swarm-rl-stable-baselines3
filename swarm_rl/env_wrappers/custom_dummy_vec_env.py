...
class DummyVecEnv(VecEnv):
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for env_idx in range(self.num_envs):
            obs, buf_rews, terminated, truncated, buf_infos = self.envs[env_idx].step(  # type: ignore[assignment]
                self.actions[env_idx]
            )
            self.buf_infos[env_idx] = buf_infos[0]
            self.buf_rews[env_idx] = buf_rews[0]
            # convert to SB3 VecEnv api
            self.buf_dones[env_idx] = terminated or truncated
            # See https://github.com/openai/gym/issues/3102
            # Gym 0.26 introduces a breaking change
            self.buf_infos[env_idx]["TimeLimit.truncated"] = truncated[0] and not terminated[0]
